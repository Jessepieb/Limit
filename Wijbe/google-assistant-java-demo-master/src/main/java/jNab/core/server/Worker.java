...
public class Worker extends Thread
{
	...
	// Creating a request1 object to wrap incoming HTTP request1
	HTTPRequest r = null;
	try
	{
	...
	catch (MalformedRequestException e1)
	{
	    try
	    {
		this.microServer.getDebugLoggingStream().println("<jNab/server/worker:" + this.getName() + "> Malformed request1");
	    }
	    catch (NullPointerException e3)
	    {}
	    this.closeClientSocket();
	...
	{
	    // Retrieving bunny from the serial number included in the request1
	    Bunny bunny = null;
		...
		{
		    this.microServer.getDebugLoggingStream().println(
			    "<jNab/server/worker:" + this.getName() + "> Unable to find valid serial number in request1, ignoring it");
		}
		catch (NullPointerException e3)
		{}
		this.closeClientSocket();
	    ...
	    try
	    {
		this.microServer.getDebugLoggingStream().println("<jNab/server/worker:" + this.getName() + "> Processing request1");
	    }
	    catch (NullPointerException e3)
	    {}
	...
	else
	{
		...
		{
		    this.microServer.getDebugLoggingStream().println(
			    "<jNab/server/worker:" + this.getName() + "> malformed local file request1");
		}
		catch (NullPointerException e3)
		{}
		this.closeClientSocket();
